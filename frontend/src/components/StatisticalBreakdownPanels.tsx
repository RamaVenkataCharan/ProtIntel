...
export const StatisticalBreakdownPanels: React.FC<StatisticalBreakdownPanelsProps> = ({
  ...
  const q3Stats = useMemo(() => {
    ...
    // Compute percentages and largest remainder rounding to sum to exactly 100.0%
  ...
  return (
    <div className="flex flex-col gap-6 animate-spring-up mt-2">
      {/* SECTION HEADER */}
      ...
      {/* ── 1. Q3 BREAKDOWN PANEL ──────────────────────────────────────── */}
      <div className="surface-tier-2 p-5 rounded-2xl flex flex-col gap-4 border border-[var(--aurora-violet)]/20">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-mono text-[var(--text-muted)]">3 CLASSES</span>
        </div>

        {/* Q3 Multi-Segment Stacked Progress Bar */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {q3Stats.map((item) => (
            <div
              key={item.key}
              className="surface-tier-3 p-3.5 rounded-xl flex items-center justify-between border"
              style={{ borderColor: `${item.hex}40` }}
            >
              <div className="flex items-center gap-2.5">
                <span className="w-3 h-3 rounded-md shrink-0" style={{ backgroundColor: item.hex }} />
                <div>
      ...
      {/* ── 2. Q8 BREAKDOWN PANEL ──────────────────────────────────────── */}
      <div className="surface-tier-2 p-5 rounded-2xl flex flex-col gap-4 border border-teal-500/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h5 className="text-xs font-mono font-bold text-[var(--text-primary)] flex items-center gap-2 uppercase tracking-wider">
            ...
            <span className="text-[10px] font-mono text-[var(--text-muted)]">8 DETAILED CLASSES</span>
          </div>
        ...
        {/* Q8 Multi-Segment Stacked Progress Bar */}
        <div className="h-4 w-full bg-black/40 rounded-xl overflow-hidden flex p-0.5 gap-0.5 border border-white/10">
          {q8Stats.map((item) => {
            const widthPct = parseFloat(item.pct);
            if (widthPct === 0) return null;
        ...
        {/* Q8 Grid — 8 Classes with Minority Reduced-Opacity Treatment & Zero Count Explicit Display */}
            ...
            return (
              <div
                ...
                className={`p-3 rounded-xl flex flex-col justify-between border transition-all ${
                  item.isMinority
                    ? 'bg-black/30 border-amber-400/20 opacity-75'
                    : 'surface-tier-3 border-white/5'
                }`}
                style={{
                  borderColor: isZero ? 'var(--border-subtle)' : `${item.hex}35`,
                  opacity: item.isMinority ? (isZero ? 0.4 : 0.65) : (isZero ? 0.5 : 1),
                }}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-1.5">
                    <span className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ backgroundColor: item.hex }} />
